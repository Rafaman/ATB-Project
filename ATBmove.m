function [event,output,dirUltimo,disUltimo]=ATBmove (mode, angolo, distance,dirUltimo,disUltimo,servoDX,servoSX)
%Creazione array
[~, rig]=size(distance);
modalita=zeros(1, 6);
...
for a=1:1:rig
    ...
    toc(cronometro);
    sensRead=ATBFAsensRead();
    arenaOut=ATBckarena();
    if sensRead<10
        output=sensRead;
        event=1;
        break;
    elseif arenaOut(4)==true
        output(1)=arenaOut(1);
        output(2)=arenaOut(2);
        output(3)=arenaOut(3);
        event=2;
